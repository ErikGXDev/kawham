...
const Modal = forwardRef(({ ...props }: ModalProps, ref) => (
  <div className={cn(props.showModal ? "block" : "hidden")}>
    <div
      {...props}
      ref={ref as any}
      className="bg-background-higher border-4 border-background-highest fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-10 rounded-xl min-w-[400px] min-h-[500px]"
    >
    ...
    <div
      className="bg-black/10 w-screen h-screen fixed left-0 top-0"
      onClick={() => {
